...
// Topic 1: Promise.all (The Team)
// Task 1: The Multi-User Fetch

async function getUserById(ids) {
    const userResponse = await fetch(`${USERS_API}/${ids}`)
    if (!userResponse.ok) {
...
// Objective: Fetch 3 different users by ID. If any one fails, the whole operation should fail.
async function getThreeUsers() {
    try {
        const userIds = [1, 2, 150];
        const responses = await Promise.all([
            getUserById(userIds[0]),
            getUserById(userIds[1]),
            getUserById(userIds[2])
        ])
        return responses
...
// Test Data: ; (Note: ID 150 might not exist, causing a failure).
