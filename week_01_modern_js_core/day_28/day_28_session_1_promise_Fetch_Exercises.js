...
async function getWidget(widget) {
    const widgets = {
        nav: "<div>WIDGET 1</div>",
        sidebar: "<div>WIDGET 2</div>",
        button: "<div>WIDGET 3</div>",
        icon: "<div>WIDGET 4</div>"
    }
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (widgets[widget]) {
                resolve(widgets[widget]);
            }
...
function getFallbackWidget() {
    return "<div>Fallback data</div>"
}
async function loadDashboard() {
    const responses = await Promise.allSettled([getWidget("nav"), getWidget("sidebar"), getWidget("ph")]);

    const widgets = responses.map((item) => {
       item.status === "fulfilled" ? item.value : getFallbackWidget();
    })
