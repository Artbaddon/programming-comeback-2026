...
**Date:** Tues Mar 24 2026
**Time:** 20:00 : 20:42
**Duration:** 50 minutes
**Review Date:** Mar 27 (Spaced Repetition)
...
**My guesses:**
- You cannot re-assign const once you assign it the first time
- Temporal Dead Zone is when you try to use a variable before it’s ready to be used.
When a scope is created, JavaScript registers the variable names first, but for let, const, and class they don’t get a value yet.
Until the code reaches their declaration, they stay “uninitialized”, and accessing them throws an error.
- in == there is type coercion but it occurs first, so it converts the type and then it does the comparison, so 5 == "5" will be true, but in === there is no type coertion so it will say false
